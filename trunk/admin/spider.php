...
********************************************/
/*
ob_end_clean();#Çå³ýÖ®Ç°µÄ»º³åÄÚÈÝ£¬ÕâÊÇ±ØÐèµÄ£¬Èç¹ûÖ®Ç°µÄ»º´æ²»Îª¿ÕµÄ»°£¬ÀïÃæ¿ÉÄÜÓÐhttpÍ·»òÕßÆäËüÄÚÈÝ£¬µ¼ÖÂºóÃæµÄÄÚÈÝ²»ÄÜ¼°Ê±µÄÊä³ö
header("Connection: close");#¸æËßä¯ÀÀÆ÷£¬Á¬½Ó¹Ø±ÕÁË£¬ÕâÑùä¯ÀÀÆ÷¾Í²»ÓÃµÈ´ý·þÎñÆ÷µÄÏìÓ¦
#¿ÉÒÔ·¢ËÍ200×´Ì¬Âë£¬ÒÔÕâÐ©ÇëÇóÊÇ³É¹¦µÄ£¬Òª²»È»¿ÉÄÜä¯ÀÀÆ÷»áÖØÊÔ£¬ÌØ±ðÊÇÓÐ´úÀíµÄÇé¿öÏÂ
ob_start();#¿ª·¢µ±Ç°´úÂë»º³å
...
//ÏÂÃæÊä³öhttpµÄÒ»Ð©Í·ÐÅÏ¢
$size=ob_get_length();
header("Content-Length: $size");
ob_end_flush();#Êä³öµ±Ç°»º³å
flush();#Êä³öPHP»º³å
...
//    ignore_user_abort(true);
	...
	//command_line,ä¸æ˜Žç™½è¿™ä¸ªå‚æ•°çš„æ„ä¹‰
	$command_line = 0;
...
//ç´¢å¼•url
	function index_url($url, $level, $site_id, $md5sum, $domain, $indexdate, $sessid, $can_leave_domain, $reindex) {
		...
		if ($url_status['state'] == 'ok') {
			...
			$file = mb_convert_encoding($file, "UTF-8", "GBK,gb2312,auto");
			$pageSize = number_format(mb_strlen($file)/1024, 2, ".", "");
		...
		/*		if ($newdomain <> $domain)
				...
				if ($OKtoIndex == 1) {
					...
					if (is_array($wordarray) && count($wordarray) > $min_words_per_page) {
						if ($md5sum == '') {
							//$fulltxt =mb_convert_encoding($fulltxt,'UTF-8','gb2312,gbk,auto');
